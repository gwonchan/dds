...
a.Fit the model
b.Conduct a 6 step hypothesis test of the slope.  That is, test the claim that the slope is significantly different from zero. Show all 6 steps and quantify your uncertainty by including a 95% confidence interval for the slope.
c.Describe the relationship between miles per gallon and the weight of the car by interpreting the slope parameter.  Again, be sure and include a 95% confidence interval in your interpretation.  
...
###T2. Using the cars.csv, conduct an internal n-fold (leave one out) cross validation of the following SLR models:
a.Model 1: 
b.Model 2: 
c.Which model is favored by this cross validation?
...
e.Use the favored model to estimate the mean mpg of cars that weigh 2000 lbs.
```{r}
#Recall
cars %>% 
  ggplot(aes(x = Weight, y = MPG)) + geom_point() + ggtitle("mtcars: mpg v. weight") + geom_smooth(method = "lm") + xlim(0,6)

# degree 1 model
fit1 = lm(MPG~Weight, data = cars)
summary(fit1)
preds1=predict(fit1)
cars %>% ggplot(aes(x = Weight, y = MPG)) + geom_point() +geom_line(data = cars, aes( x = Weight, y = preds1, col = "red"))


#degree 2 model
cars %>% ggplot(aes(x = Weight, y = MPG)) + geom_point()
cars3 = cars %>% mutate(Weight2 = Weight^2)
fit2 = lm(MPG~Weight+Weight2, cars3)
summary(fit2)

preds2 = predict(fit2)
cars %>% ggplot(aes(x = Weight, y = MPG)) + geom_point() +geom_line(data = cars, aes( x = Weight, y = preds2, col = "red"))

#deg 3 model 
cars %>% ggplot(aes(x = Weight, y = MPG)) + geom_point()
cars3 = cars %>% mutate(Weight2 = Weight^2, Weight3 = Weight^3)
fit3 = lm(MPG~Weight+Weight2+Weight3, cars3)
summary(fit3)
preds3 = predict(fit3)
cars %>% ggplot(aes(x = Weight, y = MPG)) + geom_point() +geom_line(data = cars, aes( x = Weight, y = preds3, col = "red"))
```
...
3. Using the cars.csv dataset, We would like to assess the relationship (interpret slope parameter) between mpg and horsepower.  Notice that some of the horsepowers are missing.  
	a.Impute (predict and insert) the missing horsepowers by fitting a regression model. 
 b.You may use any of the variables as regressors EXCEPT for mps (since we will later be using horsepower to predict mpg.) 
c.Assess the relationship between the mpg and the slope.  Make sure and include estimates of your uncertainty (ie. Confidence intervals.) 
...
summary(hello_cars$Horsepower)

# What rows are missing values?
missingIdx <- which(is.na(hello_cars$Horsepower))
missingIdx
